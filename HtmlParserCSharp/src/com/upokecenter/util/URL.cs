...
namespace Com.Upokecenter.Util {
  ...
  public sealed class URL {
    ...
    private static string HostParse(string stringValue) {
      ...
      try {
        // Console.WriteLine("was: %s",stringValue);
        stringValue = PercentDecode(stringValue, "utf-8");
        // Console.WriteLine("now: %s",stringValue);
      } catch (IOException) {
        return null;
      }
      return stringValue;
            ...
            if (c == '@') {
              ...
              for (var i = 0; i < bstr.Length; ++i) {
                ...
                if ((!IsUrlCodePoint(c) && c != '%') || (cp == '%' && (i + 3 >
                  bstr.Length || !IsHexDigit(bstr[index + 1]) ||

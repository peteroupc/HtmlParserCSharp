namespace com.upokecenter.util {
  ...
  public sealed class DateTimeUtility {
    ...
    private static bool isValidDay(int year, int month, int day) {
      if (year == 1582) {
        return (month == 10 && day > 4 && day < 15) ? false :
              (day <= normalDays[month]);
      } else if ((year & 3) != 0 ||
    ...
    private static void GetNormalizedPartGregorian(
  int year,  // year 1 equals 1
  int month,  // January equals 1
  long day,  // first day of month equals 1
  int[] dest) {
      int divResult;
      divResult = FloorDiv(month - 1, 12);
      year += divResult;
      month = ((month - 1) - 12 * divResult) + 1;
      int[] dayArray = ((year & 3) != 0 || (year > 1582 && year % 100 == 0 &&
      ...
      while (true) {
        ...
        if (day <= 0) {
          ...
          dayArray = ((year & 3) != 0 || (year > 1582 && year % 100 == 0 &&
            year % 400 != 0)) ? normalDays : leapDays;
    ...
    public static int[] getLocalDateComponents(long date) {
      ...
      ret[4] = (int)(FloorMod(date, 3600000L) / 60000L);
      ret[5] = (int)(FloorMod(date, 60000L) / 1000L);
      var dt = new DateTime(ret[0], ret[1], ret[2], ret[3], ret[4], ret[5]);
#if TODO
 ...
 ret[7] = 0;
  ...
  components[2]));
    // Time
    if (components[3] != 0 || components[4] != 0 ||
        components[5] != 0 || components[6] != 0) {
      b.Append(
  String.Format(
  System.Globalization.CultureInfo.InvariantCulture,
  "{0:d2}:{1:d2}:{2:d2}",
  ...
  public static String toXmlSchemaLocalDate(long time) {
    ...
    /// <param name='month'>Not documented yet.</param>
    /// <param name='day'>Not documented yet. (3).</param>
    /// <param name='hour'>Not documented yet. (4).</param>
    /// <param name='minute'>Not documented yet. (5).</param>
    /// <param name='second'>Not documented yet. (6).</param>
    /// <returns>A 64-bit signed integer.</returns>
    public static long toGmtDate(
  int year,
  int month,
  int day,
  int hour,

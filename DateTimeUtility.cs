...
public sealed class DateTimeUtility {
 ...
 numDays -= 10;
    ...
    private static void GetNormalizedPartGregorian(int year,  // year 1 equals 1
 ...
 dest[2]=(int)day;
    ...
    public static int[] getLocalDateComponents(long date) {
      ...
      DateTime dt = tz.ToLocalTime(new
        DateTime(ret[0], ret[1], ret[2], ret[3], ret[4], ret[5]));
      ret[0]=dt.Year;
      ret[1]=dt.Month;
      ret[2]=dt.Day;
      ret[3]=dt.Hour;
...
ret[8]=(int)Math.Round(TimeZone.CurrentTimeZone.GetUtcOffset(dt)
 ...
 ret[7]=0;
    ...
    if (components[3]!=0 || components[4]!=0 ||
      ...
      // Milliseconds
      if (components[6]!=0) {
  b.Append(String.Format(System.Globalization.CultureInfo.InvariantCulture,".{0:d3}",
 components[6]));
      }
    }
    // Time zone offset

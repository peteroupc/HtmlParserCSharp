...
namespace PeterO.Support
{
	...
	public sealed class SaxReader : IXmlReader {
		...
		private class AttributeList : IAttributes {
			...
			{
				if(index<0 || index>=attributes.Count)return null;
			...
			internal void Add(string qname, string uri, string localName, string value){
		...
		public void Parse(InputSource input)
		{
			if(input is InputSource<Stream>){
				...
				XmlReader reader=XmlReader.Create(
					new StreamReader(new InputStreamWrapper(iss.Source),
					                 System.Text.Encoding.GetEncoding(input.Encoding)),
					settings);
				char[] charray=null;
					...
					while(status==XmlReaderStatus.Parsing && reader.Read()){
						switch(reader.NodeType){
							case XmlNodeType.Comment:
								...
								if(lh!=null)lh.Comment(charray,0,charray.Length);
								break;
							case XmlNodeType.ProcessingInstruction:
								if(ch!=null)ch.ProcessingInstruction(
									reader.Name,reader.Value);
								break;
							case XmlNodeType.EndElement:
								if(ch!=null)ch.EndElement(reader.NamespaceURI,
								                          reader.LocalName,reader.Name);
								break;
							case XmlNodeType.DocumentType:
								if(lh!=null)lh.StartDtd(reader.Name,
								                        reader["PUBLIC"],
								                        reader["SYSTEM"]);
								if(lh!=null && status==XmlReaderStatus.Parsing)lh.EndDtd();
								break;
							case XmlNodeType.EntityReference:
								if(ch!=null)ch.SkippedEntity(reader.Name);
								break;
							...
							case XmlNodeType.SignificantWhitespace:
								charray=reader.Value.ToCharArray();
								if(ch!=null)ch.Characters(charray,0,charray.Length);
								break;
							case XmlNodeType.Whitespace:
								charray=reader.Value.ToCharArray();
								if(ch!=null)ch.IgnorableWhitespace(charray,0,charray.Length);
								break;
							case XmlNodeType.CDATA:
								lh.StartCData();
								charray=reader.Value.ToCharArray();
								if(ch!=null && status==XmlReaderStatus.Parsing)ch.Characters(charray,0,charray.Length);
								if(lh!=null && status==XmlReaderStatus.Parsing)lh.EndCData();
								break;
						}
					}
				} finally {
					if(startdoc && ch!=null)ch.EndDocument();
				}
			}
		}

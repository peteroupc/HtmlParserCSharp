...
public sealed class RDFUtility {
  public static bool areIsomorphic(ISet<RDFTriple> graph1, ISet<RDFTriple>
    graph2) {
    if (graph1 == null) {
 return graph2 == null;
}
 ...
 return false;
    ...
    /// <summary>* Converts a set of RDF Triples to a JSON _object. The
    /// _object contains all the subjects, each of which contains a
    /// dictionary of predicates for that subject, and each dictionary
    /// contains a list of objects for the subject and predicate. The
...
com.upokecenter.json.PeterO.Cbor.CBORObject valueArray = new
      ...
      string subjKey=(subject.getKind()==RDFTerm.BLANK ? "_:" : ""
)+subject.getValue();

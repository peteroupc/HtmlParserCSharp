...
public class Base64
{
	...
	private static readonly int NO_OPTIONS = 0;
	 ...
	 */
	private static readonly int URL_SAFE = 16;
	 ...
	 */
	private static readonly int ORDERED = 32;
	...
	private static readonly byte WHITE_SPACE_ENC = 251; // Indicates white space in encoding


	private static readonly byte EQUALS_SIGN_ENC = 255; // Indicates equals sign in encoding
	...
	private static byte[] decode( byte[] source, int off, int len, int options )
		...
		if( off < 0 || off + len > source.Length )
			throw new ArgumentException(
					"Source array with length "+Convert.ToString(source.Length,CultureInfo.InvariantCulture)+
					" cannot have offset of "+Convert.ToString(off,CultureInfo.InvariantCulture)+
		...
		for( i = off; i < off+len; i++ ) {  // Loop through source
			...
			if( sbiDecode >= WHITE_SPACE_ENC )  {
				...
				throw new System.IO.IOException(
						"Bad Base64 input character decimal "+Convert.ToString(source[i]&0xFF,CultureInfo.InvariantCulture)+
						" in array position "+Convert.ToString(i,CultureInfo.InvariantCulture) );
	...
	private static int decode4to3(
		...
		if( srcOffset < 0 || srcOffset + 3 >= source.Length )
			throw new ArgumentException(
					"Source array with length "+Convert.ToString(source.Length,CultureInfo.InvariantCulture)+
					" cannot have offset of "+Convert.ToString(srcOffset,CultureInfo.InvariantCulture)+" and still process four bytes." );
		if( destOffset < 0 || destOffset +2 >= destination.Length )
			throw new ArgumentException(
					"Destination array with length "+Convert.ToString(destination.Length,CultureInfo.InvariantCulture)+
					" cannot have offset of "+Convert.ToString(srcOffset,CultureInfo.InvariantCulture)+" and still store three bytes." );

...
using System;
    ...
    /// sunset times. @author Peter.</summary>
public sealed class SunriseSunset {
    /// <summary>* A rough time of day. @author Peter.</summary>
  ...
  private static double __daylen__(int year, int month, int day, double
    lon, double lat,
  ...
  private static int __sunriset__(int year, int month, int day, double lon,
    double lat,
  ...
  // **
  {
    ...
    {
      double cost;
      cost = (Math.Sin(DEGRAD*altit) - Math.Sin(DEGRAD*lat) *
        Math.Sin(DEGRAD*sdec)) /
  ...
  // The "workhorse" function
    ...
    /// <param name='lat'>Not documented yet.</param>
    /// <param name='lon'>Not documented yet.</param>
 ...
 return DayState.Night;
    ...
    if (hours<trise[0]) {
 return DayState.NightToDay;
}
    if (hours<tset[0]) {
 return DayState.Day;
}
    return (hours<twiset[0]) ? (DayState.DayToNight) : (DayState.Night);
  }
  ...
  // **
  {
    return x - 360.0 * Math.Floor(x * INV360 + 0.5);  // revolution
  }
  ...
  // **
  {
    return x - 360.0 * Math.Floor(x * INV360);  // revolution
  }

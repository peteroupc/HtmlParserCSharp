	...
	public static EncodingConfidence sniffEncoding(PeterO.Support.InputStream stream, string encoding)
		...
		while(position<count){
			if(position+4<=count &&
				...
				string charset=null;
				System.Text.StringBuilder attrName=new System.Text.StringBuilder();
				System.Text.StringBuilder attrValue=new System.Text.StringBuilder();
				position+=5;
				while(true){
		...
		// Fall back
		string lang=StringUtility.toLowerCaseAscii(System.Globalization.CultureInfo.CurrentCulture.TwoLetterISOLanguageName);
		string country=StringUtility.toUpperCaseAscii(System.Globalization.CultureInfo.CurrentCulture.Name.IndexOf('-')<0 ? "" : 
System.Globalization.CultureInfo.CurrentCulture.Name.Substring(
1+System.Globalization.CultureInfo.CurrentCulture.Name.IndexOf('-')));
		if(lang.Equals("be"))
	...
	private static int readAttribute(
			...
			int length,
			int position,
			System.Text.StringBuilder attrName,

...
"h-card" });
 ...
 "vcard", "h-card", "vevent", "h-event", "votes", "p-votes",
  ...
  private static readonly IDictionary<string, string>
    legacyLabelsMap = createLegacyLabelsMap();
...
private static void accumulateValue(PeterO.Cbor.CBORObject obj, string key, Object
 ...
 // No value elements, get the text content
    ...
    return (components[0]!=Int32.MinValue) ? (toDateTimeString(components)):
      (getDTValueContent(root));
  }
...
if (!(day <= 31)) {
 ...
 return root.getAttribute("title");
    ...
    return (StringUtility.toLowerCaseAscii(root.getLocalName()).Equals("img") &&
 ...
 return "";
  ...
  private static void getValueClassInner(IElement root, IList<IElement>
    elements) {
...
if (!StringUtility.toLowerCaseAscii(((IElement)child) .getLocalName())
  ...
  private static int isDatePattern(string value, int index, string pattern,
    int[] components) {
    int[] c = components;
